/* ================================================================
   fuel-rows.js — the metric rows of the Fuel & Power details grid.

   Shared, not duplicated: the editor (fuel-details/fuel-details.js)
   ...
   operational metrics (branches, ATMs, NPL, deposits) for the mixed bag
   of operational metrics this sector actually reports: IPPs report
   generation capacity in MW, gas transmission/distribution companies
   report throughput in MMCFD, oil marketing companies report import/
   sales volume in MT/KL. "Installed Capacity / Throughput" and "Sales /
   Distribution Volume" are deliberately unit-less (fill in per company)
   rather than three separate MW/MMCFD/MT rows, so one grid fits every
   sub-industry in the sector without empty rows for whichever units a
   given company doesn't use.
   ================================================================ */
'use strict';
...
window.FUEL_ROWS = [
  ...
  { key: 'plants',               label: 'No. of Plants/Stations/Depots',unit: '',   hint: 'integer'    },
  { key: 'installed_capacity',   label: 'Installed Capacity / Throughput', unit: '', hint: 'integer'   },
  { key: 'capacity_utilization', label: 'Capacity Utilization',         unit: '%',  hint: 'percentage' },
  { key: 'sales_volume',         label: 'Sales / Distribution Volume',  unit: '',   hint: 'integer'    },
