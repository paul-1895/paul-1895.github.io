...
let sectorMap = {};
let companyStatus = {}; // code -> {name, status, since} for closed/non-operational tickers
...
// ─── COMPANY STATUS (closed / non-operational) ────────────────────────────────
async function loadCompanyStatus() {
  if (Object.keys(companyStatus).length) return; // already loaded
  try {
    const res = await fetch('/api/company-status');
    companyStatus = (await res.json()).companies || {};
  } catch {
    companyStatus = {};
  }
}
...
function buildTicker(stocks) {
  ...
  const items = notable.map(s => {
    ...
    return `<span class="ticker-item ${dir}"><strong>${escAttrApp(s.code)}</strong> ৳${s.ltp.toFixed(1)} <em>${sign}${Math.abs(s.change).toFixed(1)}</em></span>`;
  }).join('');
...
// ─── RANKING CARDS (Top Volume / Gainers / Losers / Volatile / Closed) ────────
let rankFullLists = { volume: [], gainers: [], losers: [], volatile: [], closed: [] };

// "2026-06-25" -> "Jun 2026"; a bare year ("2016") passes through as-is.
function fmtSince(since) {
  if (!since) return '—';
  if (/^\d{4}$/.test(since)) return since;
  const d = new Date(since + 'T00:00:00');
  return isNaN(d) ? since : d.toLocaleDateString('en-US', { month: 'short', year: 'numeric' });
}
...
function buildRankingCards(stocks) {
  ...
  // Newest closures first — this card is a roster, not a today's-movers ranking,
  // so "what just got added" is more useful here than today's price change.
  const closed = stocks.filter(s => companyStatus[s.code])
    .sort((a, b) => (companyStatus[b.code].since || '').localeCompare(companyStatus[a.code].since || ''));

  rankFullLists = { volume: byVolume, gainers: byGain, losers: byLoss, volatile: byVolatile, closed };
  ...
  renderRankList('closed',   closed.slice(0, 5),     s => fmtSince(companyStatus[s.code].since));

  const countEl = document.getElementById('closed-count-badge');
...
function renderRankList(key, items, metricFn, dirClass) {
  ...
  list.innerHTML = items.map((s, i) => `
    <a class="rank-row-item" href="/company_profile/company.html?code=${encodeURIComponent(s.code)}">
      ...
      <span class="rank-code">${escAttrApp(s.code)}</span>
  ...
  if (!items.length) {
    ...
    tbody.innerHTML = items.map((s, i) => {
      ...
      return `<tr onclick="window.location.href='/company_profile/company.html?code=${encodeURIComponent(s.code)}'">
        ...
        <td class="rmt-code">${escAttrApp(s.code)}</td>
...
// Separate modal from the one above: its columns (Company/Status/Since) don't
// fit the shared table's fixed Range%/Day-Range/Volume header.
function openClosedModal() {
  const items = rankFullLists.closed || [];
  const tbody = document.getElementById('closed-modal-tbody');

  if (!items.length) {
    tbody.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:30px;color:var(--text-muted);">No data available</td></tr>';
  } else {
    tbody.innerHTML = items.map((s, i) => {
      const info = companyStatus[s.code];
      const dir  = s.change > 0 ? 'up' : s.change < 0 ? 'dn' : '';
      const sign = s.change > 0 ? '+' : '';
      const pct  = pctChange(s).toFixed(2);
      const statusLabel = info.status === 'temporary-layoff' ? 'Temporary Lay-off' : 'Closed';
      // info.name is from our own hand-maintained data/company-status.json, not
      // the dsebd.org scrape — safe to interpolate without escaping.
      return `<tr onclick="window.location.href='/company_profile/company.html?code=${encodeURIComponent(s.code)}'">
        <td class="rmt-rank">${i + 1}</td>
        <td class="rmt-code">${escAttrApp(s.code)}</td>
        <td>${info.name}</td>
        <td><span class="status-badge ${info.status}">${statusLabel}</span></td>
        <td>${fmtSince(info.since)}</td>
        <td class="rmt-num">৳${fmt(s.ltp)}</td>
        <td class="rmt-num ${dir}">${sign}${fmt(s.change)} (${sign}${pct}%)</td>
      </tr>`;
    }).join('');
  ...
  const backdrop = document.getElementById('closed-modal-backdrop');
  backdrop.classList.add('open');
  document.body.style.overflow = 'hidden';
}

function closeClosedModal(e) {
...
function renderTable(stocks) {
  ...
  tbody.innerHTML = stocks.map((s, i) => {
    ...
    return `<tr data-code="${s.code}" data-name="${escAttrApp(s.name)}" onclick="goToCompany(event,'${escAttrApp(s.code)}')" style="cursor:pointer">
      ...
      ${v.code     ? `<td class="th-code"                 data-col="code"    ><strong>${escAttrApp(s.code)}</strong></td>` : ''}
...
// Full HTML-escape (not just quotes) — this is interpolated both into
// attribute values and directly as element text content (e.g. ticker code),
// and s.code/s.sector come from the live dsebd.org scrape, not our own data.
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // (the Menu dropdown itself is now wired by shared/nav-menu.js)
  ...
  document.addEventListener('keydown', e => {
    ...
    const closedBackdrop = document.getElementById('closed-modal-backdrop');
    if (closedBackdrop && closedBackdrop.classList.contains('open')) closeClosedModal();
