...
// These values come from dsebd.org via cheerio's .text() — plain text, but
// if the source page ever visibly contained something markup-like, .text()
// preserves it verbatim and the innerHTML below would parse it as live HTML.
function escHtml(s) {
...
function makeCard(label, value, opts = {}) {
  ...
  const inner = preview
    ...
    : `<div class="${valCls}">${value === '—' ? '<span class="muted">—</span>' : escHtml(value)}</div>`;
