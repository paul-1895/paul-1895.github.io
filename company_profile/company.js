...
async function loadProfile() {
  ...
  try {
    ...
    if (window.initSubsidiaries)     window.initSubsidiaries(s.code);

    loadNewsLinks(s.code);   // defined in news.js
    loadReturns(s.code);
    loadVolatility(s.code);
    loadRiskAnalysis(s.code);

    // Load closed/non-operational status in background — data/company-status.json
    fetch(`${API}/api/company-status`)
      .then(r => r.json())
      .then(({ companies = {} } = {}) => {
        const entry = companies[s.code];
        const badgeEl = document.getElementById('p-status-badge');
        if (!badgeEl || !entry) return;
        const label = entry.status === 'temporary-layoff' ? 'Temporary Lay-off' : 'Closed / Non-Operational';
        badgeEl.textContent = label;
        badgeEl.className = `status-badge ${entry.status}`;
      ...
      .catch(() => {}); // non-critical — profile still works without it
    ...
    fetch(`${API}/api/sectors`)
      ...
      .then(map => {
        ...
        // Show Cement Details button only for Cement sector
        const cementBtn = document.getElementById('cement-details-btn');
        if (cementBtn) cementBtn.style.display = sector === 'Cement' ? 'inline-flex' : 'none';
        // Show Pharma Details button only for the Pharmaceuticals & Chemicals sector
        const pharmaBtn = document.getElementById('pharma-details-btn');
        if (pharmaBtn) pharmaBtn.style.display = sector === 'Pharmaceuticals & Chemicals' ? 'inline-flex' : 'none';
        // Show Fuel Details button only for the Fuel & Power sector
        const fuelBtn = document.getElementById('fuel-details-btn');
        if (fuelBtn) fuelBtn.style.display = sector === 'Fuel & Power' ? 'inline-flex' : 'none';
        // Show Telecom Details button only for the Telecommunication sector
        const telecomBtn = document.getElementById('telecom-details-btn');
        if (telecomBtn) telecomBtn.style.display = sector === 'Telecommunication' ? 'inline-flex' : 'none';
        // Show NBFI Details button only for the Financial Institutions sector
        const fiBtn = document.getElementById('fi-details-btn');
        if (fiBtn) fiBtn.style.display = sector === 'Financial Institutions' ? 'inline-flex' : 'none';
...
function computeRollingVolatility(records, windowDays = 30) {
  ...
  for (let i = windowDays; i < logReturns.length; i++) {
    ...
    // logReturns[i-1] is the last return folded into this window, and it
    // represents the price move ending on records[i] — so records[i] is
    // this window's "as of" date (records[i+1] would mislabel it a day late).
    series.push({ date: records[i].date, pct });
  }
  return series;
}
...
/* ----------------------------------------------------------------
   RISK & VOLATILITY ANALYSIS — VaR / CVaR / Maximum Drawdown /
   Sharpe Ratio, all computed from the same trailing-1Y daily close
   series as the Volatility card above (historical/non-parametric
   method — no assumption that returns are normally distributed).
---------------------------------------------------------------- */
const RISK_LOOKBACK_DAYS = 365;
const RISK_MIN_RETURNS = 20; // percentile-based stats are noisy below this
const RISK_FREE_RATE_PCT = 0; // no Bangladesh T-bill series on file — see caption

// 5th-percentile historical VaR: the daily loss that trailing-1Y returns
// were at least as bad as 5% of the time. Sorted ascending so the worst
// returns sit at the front.
function computeHistoricalVaR(sortedReturns, confidence = 0.95) {
  const idx = Math.max(0, Math.ceil((1 - confidence) * sortedReturns.length) - 1);
  return sortedReturns[idx];
}

// CVaR (Expected Shortfall): the average of every return at or beyond the
// VaR cutoff — how bad the bad days average out to, not just the cutoff.
function computeCVaR(sortedReturns, varThreshold) {
  const tail = sortedReturns.filter(r => r <= varThreshold);
  return tail.length ? tail.reduce((s, r) => s + r, 0) / tail.length : varThreshold;
}

// Largest peak-to-trough decline in the closing price over the window.
function computeMaxDrawdown(windowRecords) {
  let peak = windowRecords[0].close;
  let maxDD = 0;
...
async function loadRiskAnalysis(code) {
  const grid = document.getElementById('p-risk-stats');
  if (!grid) return;
  ...
  const latest = records[records.length - 1];
  const cutoff = new Date(latest.date.getTime() - RISK_LOOKBACK_DAYS * 86400000);
  const windowRecords = records.filter(r => r.date >= cutoff);
  ...
  if (dailyReturns.length < RISK_MIN_RETURNS) return;

  const volatilityPct = computeAnnualizedVol(records, latest, RISK_LOOKBACK_DAYS);

  const sorted = [...dailyReturns].sort((a, b) => a - b);
  const var95 = computeHistoricalVaR(sorted, 0.95);
  const cvar95 = computeCVaR(sorted, var95);
  const maxDrawdownPct = computeMaxDrawdown(windowRecords);
  ...
  const annualizedReturnPct = meanDaily * VOL_TRADING_DAYS_PER_YEAR * 100;
  const sharpeRatio = stdevDaily > 0
    ? ((annualizedReturnPct - RISK_FREE_RATE_PCT) / 100) / (Math.sqrt(VOL_TRADING_DAYS_PER_YEAR) * stdevDaily)
    : null;

  const setStat = (id, value, { pct = true, colorize = false } = {}) => {
    const el = document.getElementById(id);
    if (!el) return;
    if (value == null || isNaN(value)) { el.textContent = '—'; return; }
    el.textContent = pct ? `${value.toFixed(2)}%` : value.toFixed(2);
    el.className = `risk-stat-val ${colorize ? (value < 0 ? 'loss' : value > 0 ? 'gain' : '') : ''}`;
  };

  setStat('risk-volatility', volatilityPct);
  setStat('risk-var95', var95 * 100, { colorize: true });
  setStat('risk-cvar95', cvar95 * 100, { colorize: true });
  setStat('risk-mdd', maxDrawdownPct, { colorize: true });
  setStat('risk-sharpe', sharpeRatio, { pct: false, colorize: true });
}
  ...
  grid.innerHTML = stats.map(s => {
    ...
    return `
      <div class="cds-card${emptyClass}${s.clickable && display !== '—' ? ' cds-card--clickable' : ''}"
        ...
        <div class="cds-val${accentClass}">${escHtml(display)}</div>
...
// Several stat-grid values (52W High/Low, Beta, P/E, sector, etc.) come
// straight from dsebd.org via cheerio's .text() — plain text, but if the
// source page ever rendered something that *looks* like markup, .text()
// preserves it verbatim and innerHTML below would parse it as live HTML.
function escHtml(s) {
...
---------------------------------------------------------------- */
// Only renders for tickers DSEBankLogos actually has a logo for (bank
// stocks, currently 34 of 36) — every other sector's hero stays exactly
// as before. Built via DOM methods rather than innerHTML since `s.code`
// ultimately traces back to the page's own ?code= URL param.
function renderCompanyLogo(code) {
  const tile = document.getElementById('company-logo-tile');
  if (!tile) return;
  const logoUrl = window.DSEBankLogos ? window.DSEBankLogos.get(code) : null;
  tile.textContent = '';
  if (!logoUrl) { tile.style.display = 'none'; return; }
...
function populateProfile(s) {
  ...
  // Cement Details button — shown after sector data loads (see loadProfile sector fetch)
  const cementBtn = document.getElementById('cement-details-btn');
  if (cementBtn) cementBtn.href = `/cement-details/cement-details.html?code=${encodeURIComponent(s.code)}`;

  // Pharma Details button — shown after sector data loads (see loadProfile sector fetch)
  const pharmaBtn = document.getElementById('pharma-details-btn');
  if (pharmaBtn) pharmaBtn.href = `/pharma-details/pharma-details.html?code=${encodeURIComponent(s.code)}`;

  // Fuel Details button — shown after sector data loads (see loadProfile sector fetch)
  const fuelBtn = document.getElementById('fuel-details-btn');
  if (fuelBtn) fuelBtn.href = `/fuel-details/fuel-details.html?code=${encodeURIComponent(s.code)}`;

  // Telecom Details button — shown after sector data loads (see loadProfile sector fetch)
  const telecomBtn = document.getElementById('telecom-details-btn');
  if (telecomBtn) telecomBtn.href = `/telecom-details/telecom-details.html?code=${encodeURIComponent(s.code)}`;

  // NBFI Details button — shown after sector data loads (see loadProfile sector fetch)
  const fiBtn = document.getElementById('fi-details-btn');
  if (fiBtn) fiBtn.href = `/fi-details/fi-details.html?code=${encodeURIComponent(s.code)}`;
