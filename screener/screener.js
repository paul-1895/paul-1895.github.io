/* ============================================================
   ...
   ─────────────────────────────
   • Candle patterns  → daily: computed synchronously, shown instantly;
                         weekly/monthly: skeleton, patched once fetched
   ...
   ─────────────────────────────
   screenerTimeframe ('daily' | 'weekly' | 'monthly') is a single sidebar
   control that governs every candle/chart-based list — S/R, Hammer/Marubozu,
   MACD Cross, Minervini, Hilega-Milega, Liquidity Sweep, Volume Spike,
   Breakout Retest, and MACD + 50 EMA. Switching it calls
   _runTimeframeAwarePhases(), which re-fetches all of those (daily's Hammer/
   Marubozu excepted — see classifyCandlePatterns) at the new granularity.
   DDM and MF P/PV are the only two lists NOT gated by it: neither is derived
   from OHLC candles, so "weekly DDM" isn't a meaningful thing.
============================================================ */
'use strict';
...
let screenerMacdData     = {};   // code -> crossover result, for the CURRENT screenerTimeframe only
let screenerMinerviniData = {};  // code -> Minervini result (full-pass stocks only, per backend), for the CURRENT screenerTimeframe
let screenerMfPvData     = {};   // code -> price/portfolio-value ratio result (declining only, per backend) — NOT timeframe-aware (quarterly, not candle-based)
let screenerHmData       = {};   // code -> Hilega-Milega crossover result, for the CURRENT screenerTimeframe
let screenerLiqSweepData = {};   // code -> liquidity sweep result, for the CURRENT screenerTimeframe
let screenerVolSpikeData = {};   // code -> volume spike result (latest candle's volume > the one before it), for the CURRENT screenerTimeframe
let screenerRetestData   = {};   // code -> breakout retest result, for the CURRENT screenerTimeframe
let screenerMacdS1Data   = {};   // code -> MACD + 50 EMA signal standing on the latest bar, for the CURRENT screenerTimeframe
let screenerCandleData   = {};   // code -> {pattern, score, ...} from the server, weekly/monthly only (daily stays live-quote/client-side)
let screenerResults      = {
  support:[], resistance:[],
...
let screenerLoadingState = { candle: null, sr: null, ddm: null, macd: null, minervini: null, mfPv: null, hm: null, liqSweep: null, volSpike: null, retest: null, macdS1: null };
...
// 'daily' | 'weekly' | 'monthly' — the universal chart-timeframe filter,
// applied to every candle/chart-based list (all but DDM and MF P/PV, neither
// of which is derived from OHLC candles). Changing it re-fetches every
// timeframe-aware list at the new granularity via _runTimeframeAwarePhases().
let screenerTimeframe      = 'daily';
let screenerVolSpikeMinPct = 0;           // minimum latest-vs-previous-candle volume increase %, via the slider on that list
...
function classifyCandlePatterns(stocks) {
  ...
  if (screenerTimeframe === 'daily') {
    // Computed instantly from the LIVE quote, so a pattern shows up as it
    // forms intraday rather than a day late.
    stocks.forEach(s => {
    ...
    // Weekly/monthly: no "live" candle of that size to react to, so this
    // reads the server's evaluation of the latest COMPLETED aggregated bar
    // (screenerCandleData, from _fetchCandlePatterns()).
    stocks.forEach(s => {
      const hit = screenerCandleData[s.code];
      if (hit && out[hit.pattern]) out[hit.pattern].push({ stock: s, ...hit });
    });
  }
...
function classifyVolumeSpike(stocks) {
  const out = [];
  stocks.forEach(stock => {
    const v = screenerVolSpikeData[stock.code];
    if (v && v.pctChange >= screenerVolSpikeMinPct) out.push({ stock, ...v });
  });
  return out.sort((a, b) => b.pctChange - a.pctChange);
}

// Re-filter the already-fetched volume spike data at a new minimum threshold
// — purely a client-side refilter (the backend already returns every stock
// with ANY increase), so unlike screenerSetTimeframe this never refetches.
function screenerSetVolSpikeMinPct(val) {
  const pct = Math.max(0, Math.min(500, Math.round(parseFloat(val)) || 0));
  if (pct === screenerVolSpikeMinPct) return;
  screenerVolSpikeMinPct = pct;
  screenerResults.volumeSpike = classifyVolumeSpike(screenerStocksData);
  _patchNavCounts(['volumeSpike']);
  renderMainPane();
}
...
const PATTERNS = {
  ...
  minervini:{ key:'minervini', label:'Minervini Trend Template', color:'minervini',
    desc:'Meets all 7 Minervini Stage-2 uptrend criteria (50/150/200-{{unit}} SMA stack, trailing 252-bar range)',
  ...
  liqSweepBullish:{ key:'liqSweepBullish', label:'Liquidity Sweep — Bullish', color:'liq-sweep',
    desc:'Wicked below its 20-{{unit}} low then closed back above it \xb7 sell-side liquidity swept',
    ...
    emptyMsg:'No stocks wicked below a recent 20-bar low and closed back above it on the latest bar. Rescan after prices update.' },
  liqSweepBearish:{ key:'liqSweepBearish', label:'Liquidity Sweep — Bearish', color:'liq-sweep',
    desc:'Wicked above its 20-{{unit}} high then closed back below it \xb7 buy-side liquidity swept',
    ...
    badge:'🧲 SWEEP', isSR:false, isLiqSweep:true, loadGroup:'liqSweep',
    emptyMsg:'No stocks wicked above a recent 20-bar high and closed back below it on the latest bar. Rescan after prices update.' },
  volumeSpike:{ key:'volumeSpike', label:'Volume Spike', color:'vol-spike',
    desc:'Volume on the latest completed candle is higher than on the one before it',
    icon:`<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="14" width="4" height="7"/><rect x="10" y="9" width="4" height="12"/><rect x="17" y="3" width="4" height="18"/></svg>`,
    badge:'🔊 VOL', isSR:false, isVolSpike:true, loadGroup:'volSpike',
    emptyMsg:'No stocks currently show higher volume than their previous candle. Rescan after prices update.' },
  retestBullish:{ key:'retestBullish', label:'Breakout Retest \u2014 Bullish', color:'breakout-retest',
    desc:'Closed above a tested 60-{{unit}} resistance, then pulled back to it and held \xb7 resistance turned support',
    ...
    emptyMsg:'No stocks are currently retesting a broken resistance. This needs a close above a level that had held at least twice, a pullback to it within the last few bars, and no close back below \u2014 so the list is usually short.' },
  retestBearish:{ key:'retestBearish', label:'Breakout Retest \u2014 Bearish', color:'breakout-retest',
    desc:'Closed below a tested 60-{{unit}} support, then rallied back to it and was capped \xb7 support turned resistance',
...
const TF_UNIT  = { daily: 'day', weekly: 'week', monthly: 'month' };
const TF_LABEL = { daily: 'Daily', weekly: 'Weekly', monthly: 'Monthly' };

// A handful of static PATTERNS.desc strings carry a "{{unit}}" placeholder
// where a bar-count word (day/week/month) belongs, since that wording would
// otherwise go stale the moment the universal timeframe filter moves off
// daily — e.g. "20-day low" is simply wrong once the filter is set to Weekly.
function patternDesc(p) {
  return p.desc.includes('{{unit}}') ? p.desc.replace(/{{unit}}/g, TF_UNIT[screenerTimeframe]) : p.desc;
}

// Every list gated by the universal Daily/Weekly/Monthly filter (i.e.
// everything except DDM and MF P/PV, neither of which is candle-based) lives
// here — used both by the initial/rescan flow and by screenerSetTimeframe(),
// so switching timeframe re-runs exactly the same phases without touching
// DDM or MF P/PV, which don't depend on it.
async function _runTimeframeAwarePhases() {
  const tf = screenerTimeframe;
  const stale = () => tf !== screenerTimeframe; // a newer timeframe switch fired mid-flight

  // ── Candle patterns: synchronous for daily, a fetch for weekly/monthly ──
  const candleDone = (async () => {
    if (tf !== 'daily') await _fetchCandlePatterns(tf);
    if (stale()) return;
    screenerResults = { ...screenerResults, ...classifyCandlePatterns(screenerStocksData) };
    screenerLoadingState.candle = null;
  ...
  // ── MACD crossover ───────────────────────────────────────
  const macdDone = _fetchMacdCrossover(tf).then(() => {
    if (stale()) return;
  ...
  const minerviniDone = _fetchMinervini(tf).then(() => {
    if (stale()) return;
  ...
  // ── Hilega-Milega crossover ──────────────────────────────
  const hmDone = _fetchHilegaMilega(tf).then(() => {
    if (stale()) return;
  ...
  const liqSweepDone = _fetchLiquiditySweep(tf).then(() => {
    if (stale()) return;
  ...
  // ── Volume spike ─────────────────────────────────────────
  const volSpikeDone = _fetchVolumeSpike(tf).then(() => {
    if (stale()) return;
    screenerResults.volumeSpike  = classifyVolumeSpike(screenerStocksData);
    screenerLoadingState.volSpike = null;
    _patchNavCounts(['volumeSpike']);
    if (_activePatternMatchesGroup('volSpike')) renderMainPane();
  });

  // ── Breakout retest ──────────────────────────────────────
  const retestDone = _fetchBreakoutRetest(tf).then(() => {
    if (stale()) return;
  ...
  // ── MACD + 50 EMA signals ────────────────────────────────
  const macdS1Done = _fetchMacdStrategy1(tf).then(() => {
    if (stale()) return;
  ...
  await Promise.all([candleDone, srDone, macdDone, minerviniDone, hmDone, liqSweepDone, volSpikeDone, retestDone, macdS1Done]);
}
...
async function runScreenerScan() {
  ...
  screenerLoadingState = { candle: 'loading', sr: 'loading', ddm: 'loading', macd: 'loading', minervini: 'loading', mfPv: 'loading', hm: 'loading', liqSweep: 'loading', volSpike: 'loading', retest: 'loading', macdS1: 'loading' };
  _patchNavCounts(['support', 'resistance', 'bullishHammer', 'bearishHammer', 'bullishMarubozu', 'bearishMarubozu', 'ddmUndervalued', 'macdBullishCross', 'macdBearishCross', 'minervini', 'mfPvDecline', 'hmBuy', 'hmSell', 'liqSweepBullish', 'liqSweepBearish', 'volumeSpike', 'retestBullish', 'retestBearish', 'macdS1Buy', 'macdS1Exit']);
  renderMainPane();
  screenerLastScan = new Date();

  // DDM and MF P/PV sit outside _runTimeframeAwarePhases() because neither is
  // derived from OHLC candles — a dividend-discount-model value and a
  // quarterly disclosed-portfolio ratio don't have a "weekly" or "monthly"
  // version, so the universal timeframe filter simply doesn't apply to them.
  ...
  await Promise.all([_runTimeframeAwarePhases(), ddmDone, mfPvDone]);
  screenerScanRunning = false;
  _patchRescanBtn(false);
  _patchSidebarStatus();
}

// Re-run every timeframe-aware phase at a new global Daily/Weekly/Monthly
// filter, without disturbing DDM or MF P/PV (unaffected by it).
function screenerSetTimeframe(tf) {
  if (tf === screenerTimeframe || !['daily', 'weekly', 'monthly'].includes(tf)) return;
  screenerTimeframe = tf;
  ['candle', 'sr', 'macd', 'minervini', 'hm', 'liqSweep', 'volSpike', 'retest', 'macdS1']
    .forEach(g => { screenerLoadingState[g] = 'loading'; });
  _patchNavCounts([
    'support', 'resistance', 'bullishHammer', 'bearishHammer', 'bullishMarubozu', 'bearishMarubozu',
    'macdBullishCross', 'macdBearishCross', 'minervini', 'hmBuy', 'hmSell',
    'liqSweepBullish', 'liqSweepBearish', 'volumeSpike', 'retestBullish', 'retestBearish', 'macdS1Buy', 'macdS1Exit',
  ]);
  _patchTimeframeButtons();
  renderMainPane();
  _runTimeframeAwarePhases();
}

// ──────────────────────────────────────────────────────────
// FETCHERS
// ──────────────────────────────────────────────────────────
async function _fetchSR(timeframe) {
  // Auto-computed from every stock's own swing highs/lows at the requested
  // timeframe — same algorithm as the candlestick page's Support & Resistance
  // panel — rather than the old hand-entered sr_levels table, which only ever
  // covered a handful of manually-annotated stocks.
  try {
    const r = await fetch(`${API}/api/screener/sr?timeframe=${encodeURIComponent(timeframe)}`);
...
// Weekly/monthly Hammer/Marubozu — the daily lists stay client-side (see
// classifyCandlePatterns), so this is only ever called for the other two.
async function _fetchCandlePatterns(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/candle-patterns?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
    screenerCandleData = (await r.json()).results || {};
  } catch {
    screenerCandleData = {};
  }
}
...
async function _fetchMinervini(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/minervini?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
...
async function _fetchHilegaMilega(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/hilega-milega?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
...
async function _fetchLiquiditySweep(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/liquidity-sweep?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
...
async function _fetchVolumeSpike(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/volume-spike?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
    screenerVolSpikeData = (await r.json()).results || {};
  } catch {
    screenerVolSpikeData = {};
  }
}

async function _fetchBreakoutRetest(timeframe) {
  try {
    const r = await fetch(`${API}/api/screener/breakout-retest?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
...
async function _fetchMacdStrategy1(timeframe) {
  try {
    const r = await fetch(`${API}/api/macd-strategy1/signals?timeframe=${encodeURIComponent(timeframe)}`);
    if (!r.ok) throw new Error();
...
function getMetricValue(entry, p) {
  ...
  if (p.isVolSpike)     return entry.pctChange;
...
function _patchTimeframeButtons() {
  const row = document.getElementById('scr-global-tf-row');
  if (!row) return;
  row.querySelectorAll('.scr-tf-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.tf === screenerTimeframe);
  });
}
...
function buildSidebar() {
  ...
  return `
    ...
    <div class="scr-global-tf-wrap">
      <div class="scr-global-tf-label">Chart Timeframe</div>
      <div class="scr-tf-row" id="scr-global-tf-row">
        ${['daily', 'weekly', 'monthly'].map(tf => `
          <button class="scr-tf-btn${tf === screenerTimeframe ? ' active' : ''}" data-tf="${tf}"
                  onclick="screenerSetTimeframe('${tf}')">${tf[0].toUpperCase()}${tf.slice(1)}</button>`).join('')}
      </div>
    </div>
...
function buildMainPane() {
  ...
  // DDM and MF P/PV aren't derived from OHLC candles, so the universal
  // timeframe filter has no meaning for them and gets no mention here.
  const isTfAware = !p.isDDM && !p.isMfPv;
  const tfPrefix  = isTfAware ? `${TF_LABEL[screenerTimeframe]} candles \xb7 ` : '';

  const note = tfPrefix + (p.isSR
    ...
    : p.isDDM
      ...
      : p.isMacd
        ? `Crossed on the latest completed bar \xb7 fast=12 slow=26 signal=9`
        : p.isMinervini
          ? `All 7 must pass: (1) price>150\xb7200-${TF_UNIT[screenerTimeframe]} SMA (2) 150-${TF_UNIT[screenerTimeframe]} SMA>200-${TF_UNIT[screenerTimeframe]} SMA (3) 200-${TF_UNIT[screenerTimeframe]} SMA rising over the last 21 bars (4) 50-${TF_UNIT[screenerTimeframe]} SMA>150\xb7200-${TF_UNIT[screenerTimeframe]} SMA (5) price>50-${TF_UNIT[screenerTimeframe]} SMA (6) ≥ 25% above the trailing 252-bar low (7) within 25% of the trailing 252-bar high`
          : p.isMfPv
            ? `Price \xf7 sum of disclosed holdings' market value, quarter-over-quarter \xb7 covers only funds with 2+ disclosed quarters`
            : p.isHm
              ? `Crossed on the latest completed bar \xb7 RSI(9) vs its own WMA(21)`
              : p.isLiqSweep
                ? `The latest bar's wick clears the prior 20-bar range extreme by ≥ 0.1% but its close lands back inside it`
                : p.isVolSpike
                  ? `Compares each stock's latest completed candle to the one before it \xb7 volume only, price direction not considered \xb7 showing ≥ ${screenerVolSpikeMinPct}% increases`
                  : p.isRetest
                    ? `Closed ≥ 1% beyond a 60-bar level that had held ≥ 2 times, within the last 15 bars \xb7 pulled back to within 2% of it in the last 3 bars \xb7 no close 1.5% back through it`
                    : p.isMacdS1
  ...
  const volSpikeSliderHTML = p.isVolSpike ? `
    <div class="scr-slider-row">
      <label class="scr-slider-label" for="scr-volspike-slider">Min spike <span id="scr-volspike-val">${screenerVolSpikeMinPct}%</span></label>
      <input id="scr-volspike-slider" class="scr-slider" type="range" min="0" max="300" step="5"
             value="${screenerVolSpikeMinPct}"
             oninput="document.getElementById('scr-volspike-val').textContent = this.value + '%'"
             onchange="screenerSetVolSpikeMinPct(this.value)">
    </div>` : '';
  ...
  return `
    <div class="scr-main-hd" style="--rgb:${PATTERN_RGB[p.color]}">
      ...
      <div class="scr-main-title-wrap">
        ...
        <div class="scr-main-desc">${patternDesc(p)}</div>
      </div>
      ${volSpikeSliderHTML}
...
function buildSkeletonTable(p) {
  ...
  return `
    ...
    <div class="scr-skel-note">
      ...
      Fetching ${p.isSR ? 'support &amp; resistance levels' : p.isDDM ? 'dividend data' : `${TF_LABEL[screenerTimeframe].toLowerCase()} scan results`} from server&hellip;
    </div>`;
}
...
function buildRow(key, entry) {
  ...
  } else if (p.isVolSpike) {
    signalHTML = `
      <div class="scr-signal-main">🔊 +${entry.pctChange.toFixed(1)}%</div>
      <div class="scr-signal-sub">${entry.volume.toLocaleString()} vs ${entry.prevVolume.toLocaleString()} \xb7 \xd7${entry.ratio.toFixed(2)}</div>`;
  } else if (p.isRetest) {
  ...
  return `
    <tr class="scr-row" style="--rgb:${PATTERN_RGB[p.color]}" onclick="window.location='/advanced-chart/advanced-chart.html?code=${encodeURIComponent(s.code)}'">
...
window.screenerSetTimeframe     = screenerSetTimeframe;
window.screenerSetVolSpikeMinPct = screenerSetVolSpikeMinPct;
