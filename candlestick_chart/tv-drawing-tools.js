...
(function () {
  ...
  // ── Double-click-to-edit: the "lines" family types that get a TradingView-
  // style Style/Text/Coordinates/Visibility modal (tv-drawing-settings-modal.js).
  // parallelchannel is excluded — it's a 3-point object, out of scope for now.
  const EDITABLE_LINE_TYPES = new Set([
    'trendline', 'ray', 'extendedline', 'trendangle', 'hline', 'hray', 'vline', 'crossline',
  ]);
  // Types with only one anchor point — no second point to extend/measure against,
  // so the modal hides extend/midpoint/stats fields for these.
  const ONE_POINT_LINE_TYPES = new Set(['hline', 'hray', 'vline', 'crossline']);

  // color/lineWidth/lineStyle start out `null` — meaning "inherit the
  // existing theme-driven default" — rather than a hardcoded hex, so a
  // drawing placed before this feature existed keeps rendering exactly as
  // it already did until the user explicitly customizes it in the modal.
  function defaultDrawingStyle() {
    return {
      color: null, lineWidth: null, lineStyle: null,
      extendLeft: false, extendRight: false, midPoint: false, priceLabels: false,
      stats: { priceRange: false, barsRange: false, dateTimeRange: false, distance: false, angle: false, always: false, position: 'right' },
    };
  }

  // Resolves a drawing's style to concrete values for rendering (and for
  // pre-filling the edit modal) — never returns nulls.
  function effectiveStyle(d) {
    const isLight = document.documentElement.classList.contains('light-mode');
    const themeColor = isLight ? '#0891b2' : '#00f5c4';
    const s = (d && d.style) || {};
    return {
      color: s.color || themeColor,
      lineWidth: s.lineWidth || 1.5,
      lineStyle: s.lineStyle || (ONE_POINT_LINE_TYPES.has(d.type) ? 'dashed' : 'solid'),
      extendLeft: !!s.extendLeft,
      extendRight: !!s.extendRight,
      midPoint: !!s.midPoint,
      priceLabels: !!s.priceLabels,
      stats: Object.assign(
        { priceRange: false, barsRange: false, dateTimeRange: false, distance: false, angle: false, always: false, position: 'right' },
        s.stats || {}
      ),
    };
  }
  function defaultDrawingText() {
    return { enabled: false, content: '', color: '#2962FF', fontSize: 14, bold: false, italic: false, vAlign: 'top', hAlign: 'center' };
  }
  function defaultDrawingVisibility() {
    return { days: true, weeks: true, months: true };
  }

  // Lazily fills in style/text/visibility on a drawing that predates this
  // feature (or was placed before these defaults existed) — mutates in place
  // so callers can keep holding the same reference.
  function ensureDrawingExtras(d) {
    if (!d || !EDITABLE_LINE_TYPES.has(d.type)) return d;
    if (!d.style) d.style = defaultDrawingStyle();
    else {
      const def = defaultDrawingStyle();
      Object.keys(def).forEach((k) => { if (d.style[k] === undefined) d.style[k] = def[k]; });
      if (!d.style.stats) d.style.stats = def.stats;
      else Object.keys(def.stats).forEach((k) => { if (d.style.stats[k] === undefined) d.style.stats[k] = def.stats[k]; });
    }
    if (!d.text) d.text = defaultDrawingText();
    if (!d.visibility) d.visibility = defaultDrawingVisibility();
    return d;
  }
  ...
  let _dragDrawing = null;    // the userDrawings entry being repositioned via the cursor tool
  ...
  canvas.addEventListener('mousedown', (e) => {
    if (activeTool === 'cursor') {
      const id = handleSelectClick(e);
      // Horizontal lines/rays are a single price anchor — grabbing anywhere
      // along the (infinite) line and dragging vertically just updates that
      // price. Other drawing types don't support reposition-by-drag yet.
      if (id && !_lockAll) {
        const d = userDrawings.find((dr) => dr.id === id);
        if (d && (d.type === 'hline' || d.type === 'hray')) {
          _dragDrawing = d;
  ...
  canvas.addEventListener('mousemove', (e) => {
    if (activeTool === 'cursor' && _dragDrawing) {
      const { x, y } = mousePixel(e);
      const data = applyMagnet(pixelToData(x, y));
      if (data) {
        e.stopPropagation();
        _dragDrawing.pts[0].price = data.price;
        drawChart();
      }
      return;
    }
  ...
  canvas.addEventListener('dblclick', (e) => {
    ...
    if (activeTool === 'cursor') {
      ...
      if (id) {
        const d = userDrawings.find((dr) => dr.id === id);
        if (d && EDITABLE_LINE_TYPES.has(d.type) && typeof window.openDrawingSettings === 'function') {
          e.stopPropagation();
          e.preventDefault();
          _selectedId = id;
          window.openDrawingSettings(id);
        }
      }
    }
  }, true);

  // Listens on `document`, not `canvas`: a drag (drag2/freehand/measure/zoom)
  // that ends with the mouse released outside the canvas (overshoot past an
  // edge, or released over the rail/axis) would never see a canvas-scoped
  // mouseup, leaving _dragging stuck true and the preview following the
  // cursor indefinitely. mousePixel() only uses clientX/Y + canvas's own
  // bounding rect, so it stays correct regardless of the actual event target.
  ...
  // Strokes a segment from a to b, optionally extended past either endpoint
  // (same 2000px-past-canvas approach as the dedicated ray/extendedline tools).
  function drawExtendedLine(ctx, a, b, extendLeft, extendRight) {
    const dx = b.x - a.x, dy = b.y - a.y;
    const len = Math.hypot(dx, dy) || 1;
    const ext = 2000;
    const p1 = extendLeft ? { x: a.x - (dx / len) * ext, y: a.y - (dy / len) * ext } : a;
    const p2 = extendRight ? { x: b.x + (dx / len) * ext, y: b.y + (dy / len) * ext } : b;
    ctx.beginPath(); ctx.moveTo(p1.x, p1.y); ctx.lineTo(p2.x, p2.y); ctx.stroke();
  }

  function drawMiddlePointMarker(ctx, a, b, color) {
    ctx.save();
    ctx.fillStyle = color;
    ctx.beginPath();
  ...
  function drawEndpointPriceLabels(ctx, d, pxPts, color) {
    ctx.save();
    ctx.fillStyle = color;
    d.pts.forEach((p, i) => {
      const px = pxPts[i];
      if (px) ctx.fillText(p.price.toFixed(2), px.x + 6, px.y - 6);
    });
    ctx.restore();
  }

  // Draws the optional "STATS" overlay near a 2-point line (price range,
  // bars range, date/time range, distance, angle) per its style.stats flags.
  function drawLineStats(ctx, d, pxPts, es) {
    const stats = es.stats;
    if (!stats.priceRange && !stats.barsRange && !stats.dateTimeRange && !stats.distance && !stats.angle) return;
    const [a, b] = pxPts;
    const p1 = d.pts[0], p2 = d.pts[1];
    const priceDiff = p2.price - p1.price;
    const pctDiff = p1.price ? (priceDiff / p1.price) * 100 : 0;
    const bars = Math.round(p2.index - p1.index);
    const lines = [];
    if (stats.priceRange) lines.push(`${priceDiff >= 0 ? '+' : ''}${priceDiff.toFixed(2)} (${pctDiff >= 0 ? '+' : ''}${pctDiff.toFixed(2)}%)`);
    if (stats.barsRange) lines.push(`${Math.abs(bars)} bars`);
    if (stats.dateTimeRange) {
      const r = window._lastRender;
      const c1 = r && r.displayData && r.displayData[Math.round(p1.index)];
      const c2 = r && r.displayData && r.displayData[Math.round(p2.index)];
      const d1 = c1 && (c1.Date || c1.date);
      const d2 = c2 && (c2.Date || c2.date);
      if (d1 && d2) lines.push(`${d1} → ${d2}`);
    }
    if (stats.distance) lines.push(`${Math.round(Math.hypot(b.x - a.x, b.y - a.y))} px`);
    if (stats.angle) lines.push(`${(Math.atan2(-(b.y - a.y), b.x - a.x) * 180 / Math.PI).toFixed(1)}°`);
    if (!lines.length) return;
    const onLeft = stats.position === 'left';
    const anchor = onLeft ? a : b;
    const dir = onLeft ? -1 : 1;
    ctx.save();
    ctx.fillStyle = es.color;
    ctx.font = _dtFont('11px "Space Mono", monospace');
    ctx.textAlign = onLeft ? 'right' : 'left';
    lines.forEach((line, i) => ctx.fillText(line, anchor.x + dir * 8, anchor.y - 8 - i * 13));
    ctx.restore();
  }

  // ── Render one committed drawing ──────────────────────────────────
  function renderOne(ctx, width, d, lineColor, fillColor, gainColor, lossColor) {
    ensureDrawingExtras(d);
    const pxPts = d.pts.map(p => dataToPixel(p.index, p.price)).filter(Boolean);
    if (!pxPts.length) return;
    const selected = d.id === _selectedId;
    const isEditableLine = EDITABLE_LINE_TYPES.has(d.type);
    const es = isEditableLine ? effectiveStyle(d) : null;
    ctx.save();
    ctx.strokeStyle = selected ? '#ffb020' : (es ? es.color : lineColor);
    ctx.fillStyle = fillColor;
    ctx.lineWidth = selected ? Math.max(2.2, es ? es.lineWidth : 1.5) : (es ? es.lineWidth : 1.5);
    ctx.font = _dtFont('11px "Space Mono", monospace');
    if (es) {
      const dash = es.lineStyle === 'dashed' ? [5, 4] : es.lineStyle === 'dotted' ? [2, 3] : [];
    ...
    switch (d.type) {
      case 'trendline': {
        const [a, b] = pxPts;
        drawExtendedLine(ctx, a, b, es.extendLeft, es.extendRight);
        break;
      }
      case 'arrow': {
        const [a, b] = pxPts;
        ctx.beginPath(); ctx.moveTo(a.x, a.y); ctx.lineTo(b.x, b.y); ctx.stroke();
        ctx.fillStyle = ctx.strokeStyle; drawArrowhead(ctx, a, b, 9);
        break;
      }
      case 'trendangle': {
        const [a, b] = pxPts;
      ...
      case 'hray': {
        ...
        if (es.priceLabels) ctx.fillText(d.pts[0].price.toFixed(2), width - 55, pxPts[0].y - 4);
        break;
      }
      case 'vline': {
    ...
    // Style extras shared by every 2-point line-family type (trendline, ray,
    // extendedline, trendangle) — middle point, endpoint price labels, stats.
    if (isEditableLine && !ONE_POINT_LINE_TYPES.has(d.type) && pxPts.length === 2) {
      ctx.save(); ctx.setLineDash([]);
      const [a, b] = pxPts;
      if (es.midPoint) drawMiddlePointMarker(ctx, a, b, ctx.strokeStyle);
      if (es.priceLabels) drawEndpointPriceLabels(ctx, d, pxPts, es.color);
      if (es.stats.always || selected) drawLineStats(ctx, d, pxPts, es);
      ctx.restore();
    }
    // Optional text label, positioned at the drawing's first point.
    if (isEditableLine && d.text && d.text.enabled && d.text.content) {
      ctx.save();
      ctx.fillStyle = d.text.color || es.color;
      ctx.font = _dtFont(`${d.text.bold ? 'bold ' : ''}${d.text.italic ? 'italic ' : ''}${d.text.fontSize || 14}px "DM Sans", sans-serif`);
      ctx.textAlign = d.text.hAlign === 'left' ? 'left' : d.text.hAlign === 'right' ? 'right' : 'center';
      ctx.textBaseline = d.text.vAlign === 'bottom' ? 'top' : d.text.vAlign === 'middle' ? 'middle' : 'bottom';
      ctx.fillText(d.text.content, pxPts[0].x, pxPts[0].y);
      ctx.restore();
    }
  ...
  // currentTimeframe is a bare script-scope `let` ('daily'/'weekly'/'monthly')
  // declared in candlestick-data.js — same cross-file bare-identifier access
  // already used for zoomLevel/panOffset above (applyZoomBox).
  const TIMEFRAME_TO_VISIBILITY_KEY = { daily: 'days', weekly: 'weeks', monthly: 'months' };
  function isDrawingHiddenForTimeframe(d) {
    if (!d.visibility) return false;
    const key = TIMEFRAME_TO_VISIBILITY_KEY[typeof currentTimeframe !== 'undefined' ? currentTimeframe : 'daily'];
    return key ? d.visibility[key] === false : false;
  }

  // ── Small API surface for tv-drawing-settings-modal.js — userDrawings and
  // saveDrawings() are private to this IIFE, so the (separate, non-IIFE)
  // settings modal script reaches them through this object instead. ──────
  window.TVDrawingsAPI = {
    getDrawing: (id) => userDrawings.find((d) => d.id === id),
    ensureDrawingExtras,
    effectiveStyle,
    saveDrawings,
    redraw: () => drawChart(),
    EDITABLE_LINE_TYPES,
    ONE_POINT_LINE_TYPES,
  };
  ...
  window.drawUserAnnotations = function (ctx, width, priceHeight) {
    ...
    if (!_hideAll) {
      userDrawings.forEach((d) => {
        if (isDrawingHiddenForTimeframe(d)) return;
        renderOne(ctx, width, d, lineColor, fillColor, gainColor, lossColor);
      });
    }
