...
function _legendIndRow(label, parts, opts) {
  ...
  // Identify which indicator a row belongs to, in the markup. Callers already
  // pass instanceId (multi-instance overlays) or paneKey (MACD and the other
  // sub-panes); emitting them lets a host page hang per-row controls off the
  // legend — the Advanced Chart's TradingView-style hover actions — instead of
  // re-deriving the owner from the row's label text. Purely additive: nothing
  // on the candlestick page reads these, and the rendered row is unchanged.
  const idAttr   = (opts && opts.instanceId) ? ` data-instance-id="${opts.instanceId}"` : '';
  const paneAttr = (opts && opts.paneKey)    ? ` data-pane-key="${opts.paneKey}"`       : '';
  return `<div class="legend-row legend-indicator${muted ? ' legend-indicator-muted' : ''}"${idAttr}${paneAttr}>
