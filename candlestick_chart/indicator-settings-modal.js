...
function _fieldLabel(key) {
  const labels = {
    ...
    anchor: 'Anchor', anchorDate: 'Anchor Date', kPeriod: '%K Length',
    kSmooth: '%K Smoothing', dPeriod: '%D Length', fastLength: 'Fast Length',
    slowLength: 'Slow Length', signalLength: 'Signal Length',
    baseLength: 'Base Length', step: 'Step', count: 'Count',
  };
  return labels[key] || key.charAt(0).toUpperCase() + key.slice(1);
}

// Inputs whose value is a fixed set of choices rather than a free number —
// rendered as a <select> instead of a numeric field. Keyed by input name
// (matches across indicator types, so 'source' works the same for every
// def that has it, same as 'anchor' for VWAP-style anchoring).
const SELECT_FIELD_OPTIONS = {
  source: SOURCE_OPTIONS,
  anchor: ['Session', 'Week', 'Month', 'Quarter', 'Year'],
  maType: ['SMA', 'EMA', 'WMA'],
};
...
function _renderInputsTab() {
  ...
  Object.keys(def.defaultInputs).forEach(key => {
    ...
    const selectOptions = SELECT_FIELD_OPTIONS[key];
    if (selectOptions) {
      html += `
        <div class="ind-set-row">
          <label>${_fieldLabel(key)}</label>
          <select onchange="_settingsModalDraft.inputs.${key} = this.value">
            ${selectOptions.map(opt => `<option value="${opt}"${opt === value ? ' selected' : ''}>${opt}</option>`).join('')}
          </select>
        </div>`;
      ...
      // Free-text field (e.g. an anchor date) — escaped via textContent-style
      // value attribute; onchange writes the raw string back, no HTML parsing.
      html += `
        <div class="ind-set-row">
          <label>${_fieldLabel(key)}</label>
          <input type="text" value="${String(value).replace(/"/g, '&quot;')}" placeholder="YYYY-MM-DD"
                 onchange="_settingsModalDraft.inputs.${key} = this.value">
        </div>`;
    }
  });
  return html;
      ...
      html += `
        <div class="ind-set-row ind-set-style-row">
          ...
          <input type="color" value="${style[key]}" class="ind-set-color"
                 onchange="_settingsModalDraft.style.${key} = this.value; const _sw = document.getElementById('_swatch_${key}'); if (_sw) _sw.style.background = this.value;">
...
function renderIndicatorInstanceBar() {
  ...
  const instancePills = indicatorInstances.map(instance => {
    ...
    const lengthBadge = instance.inputs.length != null ? instance.inputs.length
                        ...
                        : instance.inputs.atrPeriod != null && instance.inputs.factor != null ? `${instance.inputs.atrPeriod} ${instance.inputs.factor}`
                        : Object.values(instance.inputs).filter(v => typeof v === 'number').join(' ');
