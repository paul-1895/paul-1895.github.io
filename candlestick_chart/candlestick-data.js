...
let priceOffset    = 0;
// 'daily' | 'weekly' | 'monthly' — seeded from ?timeframe= so a link from
// somewhere already showing a stock at a given timeframe (e.g. the Screener's
// Weekly/Monthly filter) opens the chart at that same timeframe, rather than
// always landing on daily and making the visitor re-click it.
let currentTimeframe = (() => {
  const tf = new URLSearchParams(window.location.search).get('timeframe');
  return (tf === 'weekly' || tf === 'monthly') ? tf : 'daily';
})();
...
// ─── Header logo tag ───────────────────────────────────────────
// candlestick.html ships with a "BPML" text pill baked into the header
// (#headerLogoTag) as its default-symbol placeholder; three call sites
// (here, candlestick-ui.js, stock-search.js) sync it to whatever symbol
// is actually on screen. Shared here so all three swap in a real bank
// logo the same way instead of just updating the placeholder text.
// Built via DOM methods rather than innerHTML — symbol can originate from
// free-text search input, so it must never be interpolated into markup.
function setHeaderLogoTag(symbol) {
  const logoEl = document.getElementById('headerLogoTag');
  if (!logoEl) return;
  const logoUrl = window.DSEBankLogos ? window.DSEBankLogos.get(symbol) : null;
  logoEl.textContent = '';
  if (!logoUrl) {
    logoEl.classList.remove('logo-tag--logo');
    logoEl.textContent = symbol;
    return;
  }
  logoEl.classList.add('logo-tag--logo');
  const img = document.createElement('img');
  img.src = logoUrl;
  img.alt = symbol;
  img.onerror = () => { logoEl.classList.remove('logo-tag--logo'); logoEl.textContent = symbol; };
...
const AVAILABLE_INDICATORS = [
  ...
  { name: 'Stochastic Oscillator',           category: 'Momentum',           description: '%K / %D oscillator'         },
  ...
  // ─── Additional MA-family overlays ───
  { name: 'Weighted Moving Average (WMA)',   category: 'Trend',              description: 'Linearly-weighted MA'        },
  { name: 'Hull Moving Average (HMA)',       category: 'Trend',              description: 'Low-lag MA'                  },
  { name: 'Double EMA (DEMA)',               category: 'Trend',              description: 'Low-lag MA'                  },
  { name: 'Triple EMA (TEMA)',               category: 'Trend',              description: 'Low-lag MA'                  },
  { name: 'Kaufman Adaptive MA (KAMA)',      category: 'Trend',              description: 'Volatility-adaptive MA'      },
  { name: 'Arnaud Legoux MA (ALMA)',         category: 'Trend',              description: 'Gaussian-weighted MA'        },
  { name: 'Volume Weighted MA (VWMA)',       category: 'Trend',              description: 'Volume-weighted MA'          },
  { name: 'MA Ribbon',                       category: 'Trend',              description: 'N stacked moving averages'   },
  { name: 'MA Cross',                        category: 'Trend',              description: 'Fast/slow MA crossover'      },
  // ─── Additional trend/volatility overlays ───
  { name: 'Parabolic SAR',                   category: 'Trend',              description: 'Stop-and-reverse trend dots' },
  { name: 'Donchian Channels',               category: 'Volatility',         description: 'Highest-high/lowest-low band'},
  { name: 'Linear Regression',               category: 'Trend',              description: 'Best-fit line + channel'     },
  { name: 'Keltner Channels',                category: 'Volatility',         description: 'EMA ± ATR band'         },
  { name: 'ATR Bands',                       category: 'Volatility',         description: 'Price ± ATR band'       },
  { name: 'Volatility Stop',                 category: 'Volatility',         description: 'ATR trailing stop-and-reverse' },
  { name: 'VWAP',                            category: 'Volume',             description: 'Volume-weighted average price' },
  { name: 'Anchored VWAP',                   category: 'Volume',             description: 'VWAP from a chosen date'     },
  // ─── Additional momentum oscillators (Williams %R/Awesome Oscillator/
  //      Chande Momentum Oscillator repoint the pre-existing placeholder
  //      rows above instead of duplicating them) ───
  { name: 'Stochastic RSI',                  category: 'Momentum',           description: 'Stochastic applied to RSI'   },
  { name: 'Commodity Channel Index (CCI)',   category: 'Momentum',           description: 'Price deviation from its mean'},
  { name: 'Rate of Change (ROC)',            category: 'Momentum',           description: '% change over N bars'        },
  { name: 'Momentum',                        category: 'Momentum',           description: 'Raw price change over N bars'},
  { name: 'Accelerator Oscillator',          category: 'Momentum',           description: 'AO minus its own SMA(5)'     },
  { name: 'Ultimate Oscillator',             category: 'Momentum',           description: 'Weighted 3-timeframe oscillator' },
  { name: 'Relative Vigor Index',            category: 'Momentum',           description: 'Close-Open vs High-Low momentum' },
  { name: 'Connors RSI',                     category: 'Momentum',           description: 'Composite short-term RSI'    },
  { name: 'Fisher Transform',                category: 'Momentum',           description: 'Gaussian-normalized price extremes' },
  { name: 'Detrended Price Oscillator (DPO)',category: 'Momentum',           description: 'Cycle-focused, non-directional' },
  { name: 'Percentage Price Oscillator (PPO)',category: 'Momentum',          description: 'MACD expressed as a %'        },
  // ─── Trend-strength sub-panes (ADX/Aroon repoint the pre-existing
  //      placeholder rows above instead of duplicating them) ───
  { name: 'Vortex Indicator',                category: 'Trend',              description: 'VI+ / VI- crossover'          },
  { name: 'TRIX',                            category: 'Trend',              description: 'Triple-smoothed EMA rate of change' },
  { name: 'Know Sure Thing (KST)',           category: 'Trend',              description: 'Smoothed 4-timeframe ROC composite' },
  { name: 'Mass Index',                      category: 'Trend',              description: 'High-Low range expansion'     },
  { name: 'Linear Regression Slope',         category: 'Trend',              description: 'Slope of the best-fit line'   },
  { name: 'Chande Forecast Oscillator',      category: 'Trend',              description: '% price vs. regression forecast' },
  // ─── Volatility sub-panes (ATR/Standard Deviation repoint the
  //      pre-existing placeholder rows above instead of duplicating) ───
  { name: 'Bollinger Band Width',            category: 'Volatility',         description: '% width between the bands'   },
  { name: 'Historical Volatility',           category: 'Volatility',         description: 'Annualized stdev of returns'  },
  { name: 'Chaikin Volatility',              category: 'Volatility',         description: '% change of the H-L range'    },
  { name: 'Ulcer Index',                     category: 'Volatility',         description: 'Depth/duration of drawdowns'  },
  { name: 'Relative Volatility Index',       category: 'Volatility',         description: 'RSI-style smoothing of stdev' },
  // ─── Volume sub-panes (OBV/Accumulation-Distribution/MFI repoint the
  //      pre-existing placeholder rows above instead of duplicating) ───
  { name: 'Volume Oscillator',               category: 'Volume',             description: '% diff of fast/slow volume EMA' },
  { name: 'Ease of Movement',                category: 'Volume',             description: 'Price move per unit of volume' },
  { name: "Force Index (Elder's)",           category: 'Volume',             description: 'Price change x volume, smoothed' },
  { name: 'Klinger Oscillator',              category: 'Volume',             description: 'Volume force + trend persistence' },
  { name: 'Negative Volume Index (NVI)',     category: 'Volume',             description: 'Cumulative on low-volume days'  },
  { name: 'Positive Volume Index (PVI)',     category: 'Volume',             description: 'Cumulative on high-volume days' },
  { name: 'Price Volume Trend (PVT)',        category: 'Volume',             description: 'Cumulative volume x % price change' },
  { name: 'Relative Volume (RVOL)',          category: 'Volume',             description: "Today's volume vs. its own average" },
  { name: 'Chaikin Money Flow',              category: 'Volume',             description: 'Money flow volume, 20-bar average' },
];
...
function _calcATRSeries(data, period) {
  ...
  for (let i = 1; i < data.length; i++) {
    ...
    if (i <= period) {
      sum += tr;
      if (i === period) { smoothed = sum / period; out[i] = smoothed; }
    } else {
...
// Looks up the button by the timeframe argument (rather than the ambient
// `event.target`) so this — and changeTimeframe() below, which reuses it —
// can run both from a live click and programmatically, e.g. to sync the UI
// to a ?timeframe= URL param applied on initial load.
function _syncTimeframeButtonUI(timeframe) {
  document.querySelectorAll('.timeframe-btn, .tv-timeframe').forEach(btn => btn.classList.remove('active'));
  const activeBtn = document.querySelector(`.timeframe-btn[onclick*="changeTimeframe('${timeframe}')"], .tv-timeframe[onclick*="changeTimeframe('${timeframe}')"]`);
...
function changeTimeframe(timeframe) {
  ...
  _syncTimeframeButtonUI(timeframe);
...
function processChartData(rows) {
  try {
    ...
    // Usually 'daily' (aggregatedData = chartData, as always) — but a
    // ?timeframe= URL param seeds currentTimeframe as non-daily on the very
    // first load, and this keeps that choice in effect instead of quietly
    // discarding it back to daily on every (re)parse of the fetched data.
    if (currentTimeframe === 'weekly')       aggregatedData = aggregateCandlesByWeek(chartData);
    else if (currentTimeframe === 'monthly') aggregatedData = aggregateCandlesByMonth(chartData);
    else                                      aggregatedData = chartData;
    if (currentTimeframe !== 'daily') {
      attachIndicators(aggregatedData);
      attachStrategies(aggregatedData);
      _syncTimeframeButtonUI(currentTimeframe);
    }
