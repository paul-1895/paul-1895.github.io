...
// Generic-framework sub-pane keys (indicators/subpane-framework.js) — each
// one is both the enabledIndicators/paneOrder entry AND the paneHeights/
// paneVZoom key for that type (identity-mapped in PANE_HEIGHT_KEY below),
// unlike the legacy MACD/RSI/Hilega-Milega panes which use a longer display
// name as the outer key and a short abbreviation as the inner one.
...
GENERIC_SUBPANE_KEYS.forEach(k => { paneHeights[k] = 100; });
...
function drawChart() {
  ...
  subPanesMeta.forEach(({ key, hKey, y, h, collapsed }) => {
    ...
    } else if (typeof drawGenericSubPane === 'function') {
      // Any indicator registered in INDICATOR_DEFS with isSubPane:true
      // (indicators/subpane-framework.js) — key is its own paneKey.
      drawGenericSubPane(ctx, key, visibleData, width, h, candleWidth, candleSpacing, padding, y);
    }
  });
  ...
  // ── One Super Model simulated trade (entry/stop/target/exit), when the
  // page was opened with ?smEntryDate=... — see sm-trade-overlay.js. Inert
  // without those params, so normal chart use is untouched.
  if (typeof drawSuperModelTrade === 'function') drawSuperModelTrade(ctx, width, TOP + paneHeights.price);
...
function _drawPaneHandles(ctx, width, padding) {
  ...
  _paneHandles.forEach(h => {
    // The price→volume boundary renders with no visible divider, so volume
    // reads as a borderless continuation of the price pane — the Advanced
    // Chart page's Lightweight Charts overlay look, cherry-picked here. It's
    // still its own resizable pane underneath (_findHandle's hit-test is
    // untouched), just without a permanent line/dots marking the seam.
    if (h.id === 'price-volume') return;
    const y = h.y;
...
function drawVolumeSection(ctx, visibleData, width, height, candleWidth, spacing, padding, startY) {
  ...
  // Minimal, Advanced-Chart-style pane: just the right-edge axis value
  // labels — no gridlines, "VOLUME" caption, or average-volume overlay,
  // cherry-picked from the Lightweight Charts histogram in advanced-chart/.
  ...
  // Same gain/loss palette as the candles, at 50% — matches the Advanced
  // Chart page's histogram opacity.
  const volUp = _dwAlpha(_dwGain('#10b981'), 0.5, 'rgba(16,185,129,0.5)');
  const volDn = _dwAlpha(_dwLoss('#ef4444'), 0.5, 'rgba(239,68,68,0.5)');
