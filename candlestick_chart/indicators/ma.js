/* ════════════════════════════════════════════════════════════
   ...
   Provides:
     ...
     calculateSMASafe(prices, period) — null-safe SMA, used by
...
// Null-safe SMA — unlike calculateSMA above, a window containing a null
// resolves to null rather than treating it as 0 (JS coerces `n + null` to
// `n`, which would silently understate the average). Needed whenever an SMA
// is applied on top of an already-null-prefixed series — e.g. smoothing a
// warm-up-limited oscillator (Stochastic %D, Stochastic RSI, Ultimate
// Oscillator) — where calculateSMA's behavior would corrupt the early bars.
function calculateSMASafe(prices, period) {
  return prices.map((_, i) => {
    if (i < period - 1) return null;
    let sum = 0;
    for (let j = i - period + 1; j <= i; j++) {
      if (prices[j] == null) return null;
      sum += prices[j];
    }
    return sum / period;
  });
