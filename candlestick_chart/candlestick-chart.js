...
(function () {
  ...
  function _applyPeriod() {
    ...
    // Candle dates are stored "YYYY/MM/DD" (slash-separated); build the cutoff
    // string in the same format using local date parts. toISOString() would
    // both use "-" separators (making the string compare always true within
    // the same year, since '/' > '-') and shift the date across UTC midnight.
...
<div class="cs-card">
  ...
  function _calcEMA(period) {
    const out = new Array(_candles.length).fill(null);
    if (_candles.length < period) return out; // not enough history to seed this EMA
    const k   = 2 / (period + 1);
