...
(function () {
  ...
  // ─── Level maths, for callers that plot rather than list ─────
  // The Advanced Chart draws these levels as price lines on the chart itself
  // (advanced-chart/ac-panels.js) and has no other way in: everything above is
  // private to this IIFE. Purely additive — this page's own behaviour is
  // unchanged. `settings` optionally overrides the panel's own lookback /
  // clusterPct / minGapPct / maxLevels for a single call, so a caller can plot
  // at different sensitivity without disturbing what the modal is showing; only
  // keys that already exist in `state` are honoured, and they are always
  // restored.
  window.SRLevels = {
    computeLevels(weekly, price, settings) {
      if (!settings) return computeLevels(weekly, price);
      const saved = {};
      ...
      try { return computeLevels(weekly, price); }
