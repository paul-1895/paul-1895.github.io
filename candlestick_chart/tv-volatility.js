...
(function () {
  ...
  function computeMonthlyMoves(records) {
    return bucketMoves(records, d => `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, '0')}`, d => `${MONTH_ABBR[d.getUTCMonth()]} ${d.getUTCFullYear()}`);
  }
  function computeYearlyMoves(records) {
    return bucketMoves(records, d => String(d.getUTCFullYear()), d => String(d.getUTCFullYear()));
  }
  ...
  function computeMonthlySeasonality(dailyMoves, threshold) {
    ...
    dailyMoves.forEach(m => {
      const b = buckets[m.date.getUTCMonth()];
      b.total++;
