...
// Default heights restored on collapse/maximize toggle. macd/rsi/hm are the
// legacy bespoke sub-panes; everything else is a generic-framework sub-pane
// (indicators/subpane-framework.js) registered via INDICATOR_DEFS' paneKey.
const _PANE_DEFAULT_H = {
  macd: 100, rsi: 100, hm: 110,
  // Trend
  adxdmi: 100, aroon: 100, linregslope: 100, vortex: 100, trix: 100, kst: 100, massindex: 100, chandeforecast: 100,
  // Momentum
  stoch: 100, stochrsi: 100, willr: 100, cci: 100, roc: 100, mom: 100, ao: 100, ac: 100, uo: 100,
  rvivigor: 100, connorsrsi: 100, cmo: 100, fisher: 100, dpo: 100, ppo: 100,
  // Volatility
  bbwidth: 100, atr: 100, stddev: 100, histvol: 100, chaikinvol: 100, ulcer: 100, rvivol: 100,
  // Volume
  obv: 100, ad: 100, cmf: 100, mfi: 100, volosc: 100, eom: 100, forceindex: 100, klinger: 100,
  nvi: 100, pvi: 100, pvt: 100, rvol: 100,
};
...
function _handleReplayBarSelect(e) {
  ...
  const fullData    = aggregatedData.length > 0 ? aggregatedData : chartData;
  const padding     = { left: 60, right: 64 };
...
// Pan momentum — a fast drag-release keeps the chart gliding for a few
// frames, decaying by friction each tick. _panVelocity is candles/ms,
// smoothed across mousemove samples while dragging; cherry-picked from the
// Advanced Chart page's Lightweight Charts pan feel (see advanced-chart/).
let _lastMoveX   = 0;
let _lastMoveT   = 0;
let _panVelocity = 0;
let _momentumRAF = null;
const MOMENTUM_MIN_VELOCITY = 0.02;   // candles/ms — below this, don't bother
const MOMENTUM_FRICTION     = 0.93;   // per-frame decay, normalized to ~60fps
const MOMENTUM_STOP_AT      = 0.001;  // candles/ms — stop the loop here
const MOMENTUM_STALE_MS     = 80;     // cursor idle longer than this before release = no flick

function _startPanMomentum() {
  // _panVelocity only updates on mousemove, so a "flick, then hold still to
  // land it" release — the cursor stopped moving well before mouseup — would
  // otherwise still fire the stale velocity from the earlier fast motion. A
  // deliberate stop should stop, not glide.
  if (performance.now() - _lastMoveT > MOMENTUM_STALE_MS) return;
  if (Math.abs(_panVelocity) < MOMENTUM_MIN_VELOCITY) return;
  let velocity = _panVelocity;
  ...
  function tick(now) {
    ...
    panOffset = panOffset + velocity * dt;
    if (!isFinite(panOffset)) panOffset = before;
    drawChart(); // re-clamps panOffset to [0, maxPan] internally
    velocity *= Math.pow(MOMENTUM_FRICTION, dt / 16.7);
    // Stop once velocity has decayed away, or once the clamp inside
    // drawChart() absorbed this tick's move entirely (hit the start/end).
    if (Math.abs(velocity) > MOMENTUM_STOP_AT && panOffset !== before) {
      _momentumRAF = requestAnimationFrame(tick);
    } else {
      panOffset = Math.round(panOffset);
      drawChart();
      _momentumRAF = null;
    }
  }
...
function setupChartInteractions() {
  ...
  window.addEventListener('mouseup', () => {
    ...
    if (wasDragging) _startPanMomentum();
  });
  ...
  canvas.addEventListener('mousemove', e => {
    ...
    // Stays live through a pan-drag too — verified that's what Lightweight
    // Charts does on the Advanced Chart page (cherry-picked here), unlike the
    // old assumption here that TradingView hides it while dragging. Still
    // suppressed mid-resize/mid-vertical-zoom, where the cursor is on a pane
    // boundary or the price axis rather than pointing at a price to read.
    if (typeof setCrosshairPosition === 'function' && !_resizing && !_isVerticalDragging) {
  ...
  window.addEventListener('mousemove', e => {
    ...
    // 1:1 cursor tracking (no artificial damping) — matches the Advanced
    // Chart page's Lightweight Charts pan feel instead of the old 40%-speed drag.
    const candleShift = Math.round(dx / approxCandleW);
    _applyPan(candleShift, dy);

    // Momentum: smoothed candles/ms velocity, used to glide the chart a
    // little further after a fast flick-release (see mouseup above).
    const now = performance.now();
    const dt  = now - _lastMoveT;
    if (dt > 4) {
      const inst = ((e.clientX - _lastMoveX) / approxCandleW) / dt;
      _panVelocity = _panVelocity * 0.7 + inst * 0.3;
      _lastMoveX = e.clientX;
      _lastMoveT = now;
    }
  });
  ...
  // Generic over every sub-pane key registered in PANE_HEIGHT_KEY (MACD/RSI/
  // Hilega-Milega plus any generic-framework indicator's paneKey) — driven by
  // the same paneOrder/enabledIndicators state the real render loop
  // (candlestick-draw.js drawChart()) uses to decide which panes are active,
  // so a newly-added sub-pane type works here with no further changes.
  function _paneAtY(canvasY) {
    ...
    const _order = (typeof paneOrder !== 'undefined') ? paneOrder : [];
    _order.filter(k => enabledIndicators.includes(k)).forEach(k => {
      const hKey = PANE_HEIGHT_KEY[k];
      if (hKey) checks.push({ key: hKey, h: (paneHeights[hKey] || 0) + SPACING });
    });
  ...
  canvas.addEventListener('touchmove', e => {
    ...
    if (e.touches.length === 2 && _isPinching) {
      ...
      const shift = Math.round(-midDx / approxCW); // 1:1 tracking, matching mouse drag
      ...
      const shift = Math.round(dx / approxCW); // 1:1 tracking, matching mouse drag
      panOffset = Math.max(0, isFinite(_tPan + shift) ? _tPan + shift : 0);
      if (_tPaneKey === 'price') {
...
// ─── Theme toggle ─────────────────────────────────────────────
// Shares 'dse-theme' (the site-wide key theme/theme.js and every other
// header toggle write to) rather than a private key, and mirrors the
// choice onto [data-theme] as well as .light-mode. Candlestick used to
// keep its own 'chartTheme' key + class with no data-theme attribute,
// which meant a global "Light"/"System" choice from Settings could set
// data-theme="light" on <html> (display-settings.js applyTheme()) and
// this page's local toggle had no way to ever clear it again — the CSS's
// ":root.light-mode, :root[data-theme=light]" rule kept winning no
// matter what the class said, so "dark mode" silently stopped working.
function setupThemeToggle() {
  const themeToggle = document.getElementById('themeToggle');
  if (!themeToggle) return;
  const legacy = localStorage.getItem('chartTheme');   // one-time migration
  let saved = localStorage.getItem('dse-theme');
  if (saved !== 'light' && saved !== 'dark') {
    saved = legacy === 'light-mode' ? 'light' : 'dark';
    localStorage.setItem('dse-theme', saved);
  }
  if (legacy) localStorage.removeItem('chartTheme');
  applyTheme(saved === 'light' ? 'light-mode' : 'dark-mode');
  themeToggle.addEventListener('click', () => {
    const root     = document.documentElement;
    const newTheme = root.classList.contains('light-mode') ? 'dark-mode' : 'light-mode';
    applyTheme(newTheme);
    localStorage.setItem('dse-theme', newTheme === 'light-mode' ? 'light' : 'dark');
  });
}
