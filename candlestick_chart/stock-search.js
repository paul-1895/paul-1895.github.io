...
// Index-level candles (historical_prices/json_files/{SYMBOL}.json) that
// aren't DSE-listed companies, so they don't belong in — and would misfire
// the live company-details lookups keyed off — stock-categories.json.
// Merged into the search list here instead.
    ...
    for (const path of paths) {
      try {
        ...
        if (response.ok) {
          ...
          const symbols = new Set(Object.keys(data.categories || {}));
          INDEX_SYMBOLS.forEach(s => symbols.add(s));
          _allStockSymbols = [...symbols].sort();
...
let _symbolClickHandlerBound = false;
function setupSymbolClickHandler() {
  if (_symbolClickHandlerBound) return; // initStockSearch() can run twice (DOMContentLoaded timer + load fallback)
  _symbolClickHandlerBound = true;
