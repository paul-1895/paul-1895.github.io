...
(function () {
  ...
  const allSharesBodyEl  = document.getElementById('tvAllSharesBody');
  const allSharesSelectEl = document.getElementById('tvAllSharesSectorSelect');
  const allSharesSearchEl = document.getElementById('tvAllSharesSearch');
  if (!watchlistBodyEl && !symbolInfoEl && !newsBodyEl && !mfHoldersBodyEl && !allSharesBodyEl) return; // not on this page
  ...
  // Selected sector filter for the All Shares panel — '' means no filter
  // (every stock). Persisted the same way chart preferences are, via the
  // localStorage helpers candlestick-data.js defines (loaded before this file).
  let allSharesSector = (typeof _loadPref === 'function') ? _loadPref('tvAllSharesSector', '') : '';
  // Free-text filter for the All Shares panel — matches against code and
  // company name. Not persisted: a stale search term hiding the board on
  // the next visit would be more confusing than starting fresh each time.
  let allSharesQuery = '';
  ...
  async function loadStaticData() {
    ...
    if (window.DSEBankLogos) window.DSEBankLogos.applyOverrides(logoByCode);
  }
  ...
  // ── All Shares: full live board, filterable by sector ──────────────────
  function populateAllSharesSectors() {
    if (!allSharesSelectEl) return;
    const sectors = [...new Set(Object.values(sectorByCode).filter(Boolean))].sort();
    const current = allSharesSelectEl.value || allSharesSector;
    allSharesSelectEl.innerHTML = '<option value="">All</option>' +
      sectors.map((s) => `<option value="${escapeHtml(s)}">${escapeHtml(s)}</option>`).join('');
    allSharesSelectEl.value = sectors.includes(current) ? current : '';
  }

  function renderAllShares() {
    if (!allSharesBodyEl) return;

    if (!stocksByCode.size) {
      allSharesBodyEl.innerHTML = '<div class="tv-sidebar-loading">Loading…</div>';
      return;
    }

    const codes = [...stocksByCode.keys()]
      .filter((code) => !allSharesSector || sectorByCode[code] === allSharesSector)
      .filter((code) => {
        if (!allSharesQuery) return true;
        const name = nameByCode[code] || '';
        return code.includes(allSharesQuery) || name.toUpperCase().includes(allSharesQuery);
      })
      .sort();

    if (!codes.length) {
      allSharesBodyEl.innerHTML = '<div class="tv-sidebar-loading">No matching stocks.</div>';
      return;
    }

    allSharesBodyEl.innerHTML = codes.map((code) => {
      const stock = stocksByCode.get(code);
      const last = stock ? stock.ltp : null;
      const pct = pctChange(stock);
      ...
      const activeCls = code === currentCode ? ' active' : '';
      return `
        <div class="tv-as-row${activeCls}" data-code="${escapeHtml(code)}">
          <span class="tv-as-symbol" title="${escapeHtml(code)}">${escapeHtml(code)}</span>
          <span class="tv-as-last">${last != null ? last.toFixed(2) : '—'}</span>
          <span class="tv-as-pct ${cls}">${pct != null ? (pct >= 0 ? '+' : '') + pct.toFixed(2) + '%' : '—'}</span>
        </div>`;
    }).join('');
  }
  ...
  // All Shares: row click switches symbol; sector dropdown re-filters.
  if (allSharesBodyEl) {
    allSharesBodyEl.addEventListener('click', (e) => {
      const row = e.target.closest('.tv-as-row');
      if (row && typeof window.reloadChartForSymbol === 'function') {
        window.reloadChartForSymbol(row.dataset.code);
      }
    });
  }
  if (allSharesSelectEl) {
    allSharesSelectEl.addEventListener('change', () => {
      allSharesSector = allSharesSelectEl.value;
      if (typeof _savePref === 'function') _savePref('tvAllSharesSector', allSharesSector);
      renderAllShares();
    });
  }
  if (allSharesSearchEl) {
    allSharesSearchEl.addEventListener('input', () => {
      allSharesQuery = allSharesSearchEl.value.trim().toUpperCase();
      renderAllShares();
    });
  }
  ...
  wireRailToggle('railAllSharesBtn', 'tvAllShares');
