...
import { renderFundPicker } from './mf-fund-picker.js';
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!FUND_CODE) {
    document.getElementById('fund-name-header').textContent = 'Mutual Fund Portfolio';
    document.getElementById('fund-code-header').textContent = 'Select a fund below';
    document.title = 'MF Portfolio — DSE';
    document.querySelector('.mf-page').style.display = 'none';
    renderFundPicker(document.getElementById('fund-picker'), {
      title: 'Mutual Fund Portfolio',
      hint: 'Pick a fund to view its quarterly portfolio.',
      buildHref: f => `/mutual_fund_portfolio/mf-portfolio.html?code=${f.code}&name=${encodeURIComponent(f.name)}`,
    });
    return;
  }
  ...
  fetch('/data/stock-logos.json')
    ...
    .then(d => {
      stockLogos = d || {};
      if (window.DSEBankLogos) window.DSEBankLogos.applyOverrides(stockLogos);
    })
    .catch(() => {});
