...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (!FUND_CODE) {
    document.getElementById('cmp-header-title').textContent = 'Quarter Comparison';
    document.getElementById('cmp-header-sub').textContent = 'Select a fund below';
    document.title = 'MF Compare — DSE';
    document.querySelector('.cmp-page').style.display = 'none';
    renderFundPicker(document.getElementById('fund-picker'), {
      title: 'Quarter Comparison',
      hint: 'Pick a fund to compare its quarters.',
      buildHref: f => `/mutual_fund_portfolio/mf-compare.html?code=${f.code}&name=${encodeURIComponent(f.name)}`,
    });
    return;
  }
